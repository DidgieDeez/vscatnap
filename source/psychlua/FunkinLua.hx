...
class FunkinLua {
	...
	public function new(script:String) {
		...
		set('week', backend.WeekData.weeksList[PlayState.storyWeek]);
		...
		Lua_helper.add_callback(lua, "setSpriteShader", function(obj:String, shader:String) {
			...
			var leObj:FlxSprite = getObjectDirectly(split[0]);
			if(split.length > 1) {
				leObj = getVarInArray(getPropertyLoopThingWhatever(split), split[split.length-1]);
			}
		...
		Lua_helper.add_callback(lua, "getShaderFloat", function(obj:String, prop:String) {
			...
			if (shader == null)
			{
				luaTrace("getShaderFloat: Shader is not FlxRuntimeShader!", false, false, FlxColor.RED);
				return null;
			}
		...
		Lua_helper.add_callback(lua, "setShaderFloat", function(obj:String, prop:String, value:Float) {
			...
			if(shader == null)
			{
				luaTrace("setShaderFloat: Shader is not FlxRuntimeShader!", false, false, FlxColor.RED);
				return false;
			}
		...
		Lua_helper.add_callback(lua, "tracehihi", function() {
			trace("hihi!");
			luaTrace("hihi!", false, false, FlxColor.RED);
		});
		Lua_helper.add_callback(lua, "setShaderFloatArray", function(obj:String, prop:String, values:Dynamic) {
			...
			if(shader == null)
			{
				luaTrace("setShaderFloatArray: Shader is not FlxRuntimeShader!", false, false, FlxColor.RED);
				return false;
			}
		...
		Lua_helper.add_callback(lua, "setShaderSampler2D", function(obj:String, prop:String, bitmapdataPath:String) {
			...
			if(shader == null)
			{
				luaTrace("setShaderSampler2D: Shader is not FlxRuntimeShader!", false, false, FlxColor.RED);
				return false;
			}
		...
		Lua_helper.add_callback(lua, "doTweenColor", function(tag:String, vars:String, targetColor:String, duration:Float, ease:String) {
			var colorNum:Dynamic = targetColor;
			if (colorNum != '0') 
				colorNum = CoolUtil.colorFromString(targetColor);
			var penisExam:Dynamic = tweenShit(tag, vars);
			if(penisExam != null) {
				var curColor:FlxColor = penisExam.color;
				curColor.alphaFloat = penisExam.alpha;
				PlayState.instance.modchartTweens.set(tag, FlxTween.color(penisExam, duration, curColor, colorNum, {ease: getFlxEaseByString(ease),
					onComplete: function(twn:FlxTween) {
						PlayState.instance.modchartTweens.remove(tag);
		...
		Lua_helper.add_callback(lua, "getColorFromHex", function(color:String) {
			var colorNum:Dynamic = color;
			if (colorNum != '0') 
				colorNum = CoolUtil.colorFromString(color);
			return colorNum;
		});
	...
	public function getShader(obj:String):FlxRuntimeShader
	{
		...
		if(split.length > 1) target = getVarInArray(getPropertyLoopThingWhatever(split), split[split.length-1]);
		else target = getObjectDirectly(split[0]);

		if(target == null)
		{
			luaTrace('Error on getting shader: Object $obj not found', false, false, FlxColor.RED);
			return null;
		}
	...
	{
		var target:Dynamic = tweenShit(tag, vars);
		if(target != null) {
			PlayState.instance.modchartTweens.set(tag, FlxTween.tween(target, tweenValue, duration, {ease: getFlxEaseByString(ease),
				onComplete: function(twn:FlxTween) {
					PlayState.instance.modchartTweens.remove(tag);
					PlayState.instance.callOnLuas('onTweenCompleted', [tag, vars]);
				}
