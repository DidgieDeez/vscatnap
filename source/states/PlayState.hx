...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(FileSystem.exists(Paths.modFolders('stages/' + curStage + '.lua'))) {
			luaArray.push(new FunkinLua(Paths.modFolders('stages/' + curStage + '.lua')));
		} else if (FileSystem.exists(Paths.getSharedPath('stages/' + curStage + '.lua'))) {
			luaArray.push(new FunkinLua(Paths.getSharedPath('stages/' + curStage + '.lua')));
		}
		#else
		luaArray.push(new FunkinLua(Paths.getSharedPath('stages/' + curStage + '.lua')));
		#end
		...
		botplayTxt = new FlxText(400, timeBar.y + 55, FlxG.width - 800, "HERETIC", 32);
		...
		for (event in eventPushedMap.keys())
		{
			...
			if(FileSystem.exists(luaToLoad))
			{
				trace('event ' + luaToLoad + ' should be loaded');
				luaArray.push(new FunkinLua(luaToLoad));
			}
			else
			{
	...
	{
		var ret:Dynamic = callOnLuas('preUpdateScore', [miss], true);
		if (ret == FunkinLua.Function_Stop)
			return;
	...
	private function keyPressed(key:Int)
	{
		...
		var ret:Dynamic = callOnLuas('onKeyPressPre', [key]);
	...
	private function keyReleased(key:Int)
	{
		...
		var ret:Dynamic = callOnLuas('onKeyReleasePre', [key]);
		if(ret == FunkinLua.Function_Stop) return;
	...
	{
		callOnLuas('opponentNoteHitPre', [notes.members.indexOf(note), Math.abs(note.noteData), note.noteType, note.isSustainNote]);
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		callOnLuas('goodNoteHitPre', [notes.members.indexOf(note), leData, leType, isSus]);
